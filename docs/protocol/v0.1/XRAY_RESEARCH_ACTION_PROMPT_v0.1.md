...
Then investigate the material at `SOURCE_URL` by executing **Stages 1 through 9 of the protocol exactly as written**.
...
Execute **Stages 1–9 only**.

Do **not** perform Stage 10 (`SYNTHESIZE`) or Stage 11 (`ACT`).
...
> **what was claimed → where it came from → what supports it → what challenges it → what remains missing → what would change the finding.**
